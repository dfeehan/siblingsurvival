...
#   ASDR.agg = 36/353  per year  (~0.1020)
#   ASDR.ind =  4/29   per year  (~0.1379)
...
#
#   Total deaths     = 3
#   Total exposure   = (60+60+31+60+11+11+60+60) months = 353 months
#                    = 353/12 person-years  (after exp.scale = 1/12)
#
#   A sibling who dies contributes the month of death: end_obs = dod + 1, so C
#   (died 570) contributes 571-540 = 31 months and E, F (died 550) contribute
#   11 each. This is what prep_dhs_sib_histories() produces and what the DHS
#   reference implementation counts (mexp = last - first + 1).
#
#   ASDR.agg = 3 / (353/12) = 36/353
test_that("sibling_estimator: aggregate ASDR matches hand-computed value", {
  ...
  expect_equal(row$asdr.hat, 36/353, tolerance = 1e-10)
})
...
#     y.Ncell.ind = (31/12)*(1/2) + (60/12)*1 = 31/24 + 5 = 151/24
#
#   Ego 3 (E,F both dead at 550):  y.F=0 → vis = 1/(0+1) = 1 each
#     y.Dcell.ind = 1*1 + 1*1 = 2
#     y.Ncell.ind = (11/12)*1 + (11/12)*1 = 22/12 = 44/24
#
...
#   Sum y.Ncell.ind = 120/24 + 151/24 + 44/24 + 120/24 = 435/24 person-years
#
#   ASDR.ind = 2.5 / (435/24) = 60/435 = 4/29
test_that("sibling_estimator: individual ASDR matches hand-computed value", {
  ...
  expect_equal(row$asdr.hat, 4/29, tolerance = 1e-10)
})
...
test_that("sibling_estimator works correctly when called via purrr::imap_dfr", {
  ...
  expect_equal(city_a$asdr.hat, 36/353, tolerance = 1e-10)
