...
bootweights <- surveybootstrap::rescaled.bootstrap.weights(survey.design = ~ psu + stratum(stratum),
                                                           ...
                                                           weights='ego.weight', # column with the sampling weight
...
ic.checks <- sib_ic_checks(test.esc.dat,
                           ego.dat=ex.ego,
                           ego.id='caseid',
                           sib.id='sibid',
                           sib.frame.indicator='in.F',
                           sib.cell.vars=c('sib.age', 'sib.sex'),
                           ego.cell.vars=c('ego.agecat5', 'ego.sex'),
...
ggplot(ic.checks$ic.agg) +
  ...
  geom_pointrange(aes(x=ego.agecat5,
                      y=diff_mean,
                      ymin=diff_ci_low,
                      ymax=diff_ci_high)) +
  theme_minimal() +
...
* more generally, sibling_estimator now has hard-coded 'agelabel' references -- but it shouldn't. need to fix
