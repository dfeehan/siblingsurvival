...
These sibs with missing `in.F` turn out not to have ages recorded. 
Since we need to be able to determine whether or not each sibling is on in the
frame population, we will drop them from the analysis.
...
* to produce estimates for the 5-year window before each survey interview (so, slightly different for each respondent)
...
ggplot(ex_ests$asdr.ind) +
  geom_line(aes(x=sib.age, y=1000*asdr.hat, color=sex, group=sex)) +
  theme_minimal() +
  scale_y_log10() + 
  ggtitle('individual visibility estimator, 5yr before survey')
```
...
ggplot(ex_ests$asdr.agg) +
  ...
  theme_minimal() +
  scale_y_log10() + 
  ggtitle('aggregate visibility estimator, 5yr before survey')
```
...
ggplot(compare) +
  geom_line(aes(x=sib.age, y=1000*asdr.hat, color=sex, linetype=estimator, group=interaction(estimator, sex))) +
  theme_minimal() +
  #facet_grid(sex ~ .) +
  facet_grid(. ~ sex) +
  scale_y_log10() + 
  ggtitle('both estimators, 5yr before survey')
...
(NOTE: this is a little slow; it takes about 30 minutes or so on a 2018 MBP for 1000 resamples.)

```{r}
set.seed(101010)

tic('running bootstrap')
## this will take a little while -- for 1000 reps, it takes about 30 minutes on a 2018 Macbook Pro
num.boot.reps <- 1000
bootweights <- surveybootstrap::rescaled.bootstrap.weights(survey.design = ~ psu + stratum(stratum),
                                                           ...
                                                           weights='indweight',  # column with the sampling weight
...
(NOTE: this is also slow; it takes about TODO minutes or so on a 2018 MBP when `bootweights` has 1000 resamples.)
...
ex_boot_ests <- sibling_estimator(sib.dat = ex.sib,
                                  ...
                                  sib.sex = 'sex',
                                  cell.config=cc,
                                  boot.weights=bootweights,  # to get sampling uncertainty, we pass boot.weights into sibling_estimator
...
ggplot(ex_boot_ests$asdr.ind) +
  geom_ribbon(aes(x=sib.age, ymin=1000*asdr.hat.ci.low, ymax=1000*asdr.hat.ci.high, fill=sex, group=sex), alpha=.2) +
  geom_line(aes(x=sib.age, y=1000*asdr.hat, color=sex, group=sex)) +
  theme_minimal() +
  scale_y_log10() + 
  ggtitle('individual visibility estimator, 5yr before survey')
